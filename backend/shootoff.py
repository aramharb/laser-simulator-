...
class MainWindow:
    def refresh_frame(self, *args):
        ...
        webcam_frame = self._webcam_frame

        if self._calibrate_projector:
            webcam_frame = self._projector_calibrator.calibrate_projector(webcam_frame)

        # OpenCV reads the frame in BGR, but PIL uses RGB.
    ...
    def detect_shots(self):
        ...
        laser_intensity = self._preferences[configurator.LASER_INTENSITY]
        if (self._shot_detector is None
            or self._shot_detector_frame_shape != (frame_width, frame_height)
            or self._shot_detector_intensity != laser_intensity):
            brightness_ratio = float(laser_intensity) / 255.0
            self._shot_detector = JavaShotDetector(
                frame_width,
                frame_height,
                brightness_increase_ratio=brightness_ratio,
            )
            self._shot_detector_frame_shape = (frame_width, frame_height)
            self._shot_detector_intensity = laser_intensity

        # Keep the interference warning based on the legacy threshold
        frame_bw = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        (_, frame_thresh) = cv2.threshold(frame_bw, laser_intensity, 255, cv2.THRESH_BINARY)
        if not self._seen_interference:
            self.detect_interfence(frame_thresh)

        fps = int(round(self._camera_fps)) if self._camera_fps and self._camera_fps > 0 else 30
        shots = self._shot_detector.process_frame(frame, fps=fps)
        ...
        for shot in shots:
            ...
            if self._preferences[configurator.IGNORE_LASER_COLOR] in laser_color:
                continue

            x = int(round(shot.get("x", 0)))
            y = int(round(shot.get("y", 0)))
    ...
    def detect_interfence(self, image_thresh):
        ...
        percent_dark = float(brightness_hist[0][0]) / float(image_thresh.size)
    ...
    def load_training(self, plugin):
        ...
        spec = importlib.util.spec_from_file_location("__init__", os.path.join(plugin[1], "__init__.py"))
    ...
    def create_training_list(self, menu, func):
        ...
        for candidate in plugin_candidates:
            ...
            if (not os.path.isdir(plugin_location) or
                ...
                continue
            spec = importlib.util.spec_from_file_location("__init__", os.path.join(plugin_location, "__init__.py"))
            module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(module)
            training_info = module.get_info()
            # Simulating find_module return for compatibility with load_training expectations
            plugin_info = (None, plugin_location, None) 
    ...
    def __init__(self, config):
        ...
        self._shot_detector_frame_shape = None
        self._shot_detector_intensity = None
        ...
        if self._cv.isOpened():
            ...
            if width < 640 and height < 480:
                ...
                self._cv.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
                self._cv.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
                width = self._cv.get(cv2.CAP_PROP_FRAME_WIDTH)
                height = self._cv.get(cv2.CAP_PROP_FRAME_HEIGHT)
            ...
            if fps <= 0:
                self._camera_fps = 30.0
                self._logger.info("Couldn't get webcam FPS, defaulting to 30 ms refresh interval.")
            else:
                self._camera_fps = float(fps)
                global FEED_FPS
                FEED_FPS = int(round(1000.0 / fps))
                self._logger.info("Feed FPS set to %.2f, refresh interval %d ms.", fps, FEED_FPS)
