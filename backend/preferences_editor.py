...
DEFAULT_VIDCAM = 1
DEFAULT_SHOT_LUMINANCE_MODE = "combined"
DEFAULT_SHOT_EXCESSIVE_BRIGHTNESS_RATIO = 0.96
DEFAULT_SHOT_MIN_DIMENSION = 8
DEFAULT_SHOT_DYNAMIC_RATIO_CAP = 0.25
DEFAULT_SHOT_USE_DYNAMIC_THRESHOLD = True
DEFAULT_SHOT_USE_EXCESSIVE_MASK = True
DEFAULT_SHOT_USE_VARIANCE_THRESHOLD = True
DEFAULT_SHOT_VARIANCE_ALPHA = 0.05
DEFAULT_SHOT_VARIANCE_K = 2.5
DEFAULT_SHOT_USE_TEMPORAL_FILTER = True
DEFAULT_SHOT_TEMPORAL_WINDOW = 3
DEFAULT_SHOT_TEMPORAL_MIN_FRAMES = 1
...
class PreferencesEditor():
    ...
    def map_configuration():
        ...
        def read_bool(section, key, default):
            try:
        ...
        def read_int(section, key, default):
            try:
                return config.getint(section, key)
            except (ConfigParser.NoOptionError, ValueError):
                return default

        def read_float(section, key, default):
            try:
                return config.getfloat(section, key)
            except (ConfigParser.NoOptionError, ValueError):
                return default

        def read_str(section, key, default):
            try:
        ...
        if os.path.exists("settings.conf"):
            ...
            try:
                vidcam_val = config.get("ShootOFF", configurator.VIDCAM)
                if vidcam_val.isdigit():
                    preferences[configurator.VIDCAM] = int(vidcam_val)
                else:
                    preferences[configurator.VIDCAM] = vidcam_val
            except ConfigParser.NoOptionError:
            ...
            preferences[configurator.SHOT_LUMINANCE_MODE] = read_str(
                "ShootOFF",
                configurator.SHOT_LUMINANCE_MODE,
                DEFAULT_SHOT_LUMINANCE_MODE,
            )
            preferences[configurator.SHOT_EXCESSIVE_BRIGHTNESS_RATIO] = read_float(
                "ShootOFF",
                configurator.SHOT_EXCESSIVE_BRIGHTNESS_RATIO,
                DEFAULT_SHOT_EXCESSIVE_BRIGHTNESS_RATIO,
            )
            preferences[configurator.SHOT_MIN_DIMENSION] = read_int(
                "ShootOFF",
                configurator.SHOT_MIN_DIMENSION,
                DEFAULT_SHOT_MIN_DIMENSION,
            )
            preferences[configurator.SHOT_DYNAMIC_RATIO_CAP] = read_float(
                "ShootOFF",
                configurator.SHOT_DYNAMIC_RATIO_CAP,
                DEFAULT_SHOT_DYNAMIC_RATIO_CAP,
            )
            preferences[configurator.SHOT_USE_DYNAMIC_THRESHOLD] = read_bool(
                "ShootOFF",
                configurator.SHOT_USE_DYNAMIC_THRESHOLD,
                DEFAULT_SHOT_USE_DYNAMIC_THRESHOLD,
            )
            preferences[configurator.SHOT_USE_EXCESSIVE_MASK] = read_bool(
                "ShootOFF",
                configurator.SHOT_USE_EXCESSIVE_MASK,
                DEFAULT_SHOT_USE_EXCESSIVE_MASK,
            )
            preferences[configurator.SHOT_USE_VARIANCE_THRESHOLD] = read_bool(
                "ShootOFF",
                configurator.SHOT_USE_VARIANCE_THRESHOLD,
                DEFAULT_SHOT_USE_VARIANCE_THRESHOLD,
            )
            preferences[configurator.SHOT_VARIANCE_ALPHA] = read_float(
                "ShootOFF",
                configurator.SHOT_VARIANCE_ALPHA,
                DEFAULT_SHOT_VARIANCE_ALPHA,
            )
            preferences[configurator.SHOT_VARIANCE_K] = read_float(
                "ShootOFF",
                configurator.SHOT_VARIANCE_K,
                DEFAULT_SHOT_VARIANCE_K,
            )
            preferences[configurator.SHOT_USE_TEMPORAL_FILTER] = read_bool(
                "ShootOFF",
                configurator.SHOT_USE_TEMPORAL_FILTER,
                DEFAULT_SHOT_USE_TEMPORAL_FILTER,
            )
            preferences[configurator.SHOT_TEMPORAL_WINDOW] = read_int(
                "ShootOFF",
                configurator.SHOT_TEMPORAL_WINDOW,
                DEFAULT_SHOT_TEMPORAL_WINDOW,
            )
            preferences[configurator.SHOT_TEMPORAL_MIN_FRAMES] = read_int(
                "ShootOFF",
                configurator.SHOT_TEMPORAL_MIN_FRAMES,
                DEFAULT_SHOT_TEMPORAL_MIN_FRAMES,
            )
        else:
            preferences[configurator.DETECTION_RATE] = DEFAULT_DETECTION_RATE
            preferences[configurator.LASER_INTENSITY] = DEFAULT_LASER_INTENSITY
            ...
            preferences[configurator.USE_MALFUNCTIONS] = DEFAULT_USE_MALFUNCTIONS
            preferences[configurator.MALFUNCTION_PROBABILITY] = DEFAULT_MALFUNCTION_PROBABILITY
            preferences[configurator.SHOT_LUMINANCE_MODE] = DEFAULT_SHOT_LUMINANCE_MODE
            preferences[configurator.SHOT_EXCESSIVE_BRIGHTNESS_RATIO] = DEFAULT_SHOT_EXCESSIVE_BRIGHTNESS_RATIO
            preferences[configurator.SHOT_MIN_DIMENSION] = DEFAULT_SHOT_MIN_DIMENSION
            preferences[configurator.SHOT_DYNAMIC_RATIO_CAP] = DEFAULT_SHOT_DYNAMIC_RATIO_CAP
            preferences[configurator.SHOT_USE_DYNAMIC_THRESHOLD] = DEFAULT_SHOT_USE_DYNAMIC_THRESHOLD
            preferences[configurator.SHOT_USE_EXCESSIVE_MASK] = DEFAULT_SHOT_USE_EXCESSIVE_MASK
            preferences[configurator.SHOT_USE_VARIANCE_THRESHOLD] = DEFAULT_SHOT_USE_VARIANCE_THRESHOLD
            preferences[configurator.SHOT_VARIANCE_ALPHA] = DEFAULT_SHOT_VARIANCE_ALPHA
            preferences[configurator.SHOT_VARIANCE_K] = DEFAULT_SHOT_VARIANCE_K
            preferences[configurator.SHOT_USE_TEMPORAL_FILTER] = DEFAULT_SHOT_USE_TEMPORAL_FILTER
            preferences[configurator.SHOT_TEMPORAL_WINDOW] = DEFAULT_SHOT_TEMPORAL_WINDOW
            preferences[configurator.SHOT_TEMPORAL_MIN_FRAMES] = DEFAULT_SHOT_TEMPORAL_MIN_FRAMES
            ...
            config.set("ShootOFF", configurator.SHOT_LUMINANCE_MODE,
                str(preferences[configurator.SHOT_LUMINANCE_MODE]))
            config.set("ShootOFF", configurator.SHOT_EXCESSIVE_BRIGHTNESS_RATIO,
                str(preferences[configurator.SHOT_EXCESSIVE_BRIGHTNESS_RATIO]))
            config.set("ShootOFF", configurator.SHOT_MIN_DIMENSION,
                str(preferences[configurator.SHOT_MIN_DIMENSION]))
            config.set("ShootOFF", configurator.SHOT_DYNAMIC_RATIO_CAP,
                str(preferences[configurator.SHOT_DYNAMIC_RATIO_CAP]))
            config.set("ShootOFF", configurator.SHOT_USE_DYNAMIC_THRESHOLD,
                str(preferences[configurator.SHOT_USE_DYNAMIC_THRESHOLD]))
            config.set("ShootOFF", configurator.SHOT_USE_EXCESSIVE_MASK,
                str(preferences[configurator.SHOT_USE_EXCESSIVE_MASK]))
            config.set("ShootOFF", configurator.SHOT_USE_VARIANCE_THRESHOLD,
                str(preferences[configurator.SHOT_USE_VARIANCE_THRESHOLD]))
            config.set("ShootOFF", configurator.SHOT_VARIANCE_ALPHA,
                str(preferences[configurator.SHOT_VARIANCE_ALPHA]))
            config.set("ShootOFF", configurator.SHOT_VARIANCE_K,
                str(preferences[configurator.SHOT_VARIANCE_K]))
            config.set("ShootOFF", configurator.SHOT_USE_TEMPORAL_FILTER,
                str(preferences[configurator.SHOT_USE_TEMPORAL_FILTER]))
            config.set("ShootOFF", configurator.SHOT_TEMPORAL_WINDOW,
                str(preferences[configurator.SHOT_TEMPORAL_WINDOW]))
            config.set("ShootOFF", configurator.SHOT_TEMPORAL_MIN_FRAMES,
                str(preferences[configurator.SHOT_TEMPORAL_MIN_FRAMES]))
    ...
    def save_preferences(self):
        ...
        self._config_parser.set("ShootOFF", configurator.SHOT_LUMINANCE_MODE,
             str(self._preferences[configurator.SHOT_LUMINANCE_MODE]))
        self._config_parser.set("ShootOFF", configurator.SHOT_EXCESSIVE_BRIGHTNESS_RATIO,
             str(self._preferences[configurator.SHOT_EXCESSIVE_BRIGHTNESS_RATIO]))
        self._config_parser.set("ShootOFF", configurator.SHOT_MIN_DIMENSION,
             str(self._preferences[configurator.SHOT_MIN_DIMENSION]))
        self._config_parser.set("ShootOFF", configurator.SHOT_DYNAMIC_RATIO_CAP,
             str(self._preferences[configurator.SHOT_DYNAMIC_RATIO_CAP]))
        self._config_parser.set("ShootOFF", configurator.SHOT_USE_DYNAMIC_THRESHOLD,
             str(self._preferences[configurator.SHOT_USE_DYNAMIC_THRESHOLD]))
        self._config_parser.set("ShootOFF", configurator.SHOT_USE_EXCESSIVE_MASK,
             str(self._preferences[configurator.SHOT_USE_EXCESSIVE_MASK]))
        self._config_parser.set("ShootOFF", configurator.SHOT_USE_VARIANCE_THRESHOLD,
             str(self._preferences[configurator.SHOT_USE_VARIANCE_THRESHOLD]))
        self._config_parser.set("ShootOFF", configurator.SHOT_VARIANCE_ALPHA,
             str(self._preferences[configurator.SHOT_VARIANCE_ALPHA]))
        self._config_parser.set("ShootOFF", configurator.SHOT_VARIANCE_K,
             str(self._preferences[configurator.SHOT_VARIANCE_K]))
        self._config_parser.set("ShootOFF", configurator.SHOT_USE_TEMPORAL_FILTER,
             str(self._preferences[configurator.SHOT_USE_TEMPORAL_FILTER]))
        self._config_parser.set("ShootOFF", configurator.SHOT_TEMPORAL_WINDOW,
             str(self._preferences[configurator.SHOT_TEMPORAL_WINDOW]))
        self._config_parser.set("ShootOFF", configurator.SHOT_TEMPORAL_MIN_FRAMES,
             str(self._preferences[configurator.SHOT_TEMPORAL_MIN_FRAMES]))
