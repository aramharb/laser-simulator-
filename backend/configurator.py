...
class Configurator():
    ...
    def _check_vidcam(self, vidcam):
        try:
            # Try to treat it as a local device index
            value = int(vidcam)
            if value < 0:
                raise ValueError
            return value
        except ValueError:
            # If not an int, return as string (e.g., an RTSP or HTTP URL for a phone)
            return vidcam

    def _check_ignore_laser_color(self, ignore_laser_color):
        ignore_laser_color = ignore_laser_color.lower()
        if ignore_laser_color not in ("none", "red", "green"):
            raise argparse.ArgumentTypeError("IGNORE_LASER_COLOR must be a string " +
                "equal to \"none\", \"green\", or \"red\" without quotes")
    ...
    def __init__(self, argv=None):
        ...
        if args.vidcam is not None:
            # Only cast to int if it's purely numeric
            if str(args.vidcam).isdigit():
                preferences[VIDCAM] = int(args.vidcam)
            else:
                preferences[VIDCAM] = args.vidcam
